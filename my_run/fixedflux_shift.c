...
int main(void) {
  ...
  //f.sigma = 0.;
  f.sigma = rho2*sq(u_ref)*2.0*R0/WE;
  ...
  double mllist[] = {5, 6, 7, 8, 9};
  double dtlist[] = {2.5e-3, 1.25e-3, 6.25e-4, 3.125e-4, 1.5625e-4};
  //double mllist[] = {6};
...
event movie (t += 0.125*f_r) {
  ...
  draw_vof ("f", lw = 1.5);
  //squares ("u.x", linear = true, n = {0,1}, alpha = -L0/2.0, map = cool_warm, spread = -1);
