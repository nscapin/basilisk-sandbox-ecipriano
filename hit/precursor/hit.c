...
#include "sandbox/adapt_wavelet_leave_interface.h"   
...
double t_out_stat = 64.00;  // output frequency of global observables 
double t_out_dpst = 16.00;  // output frequency for dump restarting files 
double t_out_dpbk = 4.000;  // output frequency back-up restarting files
double t_out_mov  = 16.00;  // output frequency of movie
double t_out_tag  = 16.00;  // output frequency of tagging
...
event init (i = 0) {
  ...
  if (single_ph == 1) {
    ...
    fprintf(stderr, "Two-phase simulation\n"), fflush (stderr);
    if (restore (file = "restart_sp.bin")) { // a precursor
      ...
      draw_vof("f");
      squares ("omg", spread = 5, linear = true, map = jet, 
               //cbar = true, label = "Omega", lscale = 0.9, format = "%.2f", pos = {-0.95,-0.95}, horizontal = false,
               alpha = 1.e-6, n = {1, 0 , 0});
      squares ("omg", spread = 5, linear = true, map = jet, alpha = 1.e-6, n = {0, 1 , 0});
      squares ("omg", spread = 5, linear = true, map = jet, alpha = 1.e-6, n = {0, 0 , 1});
...
event logfile (i += 10) {
  ...
  if (pid() == 0) {
    ...
    fprintf (fd, "%8E %09d %8E %8E %8E %8E %8E %8E %8E\n",
                  t, i, vd, ke, vel_rms, vd/(3.0*sq(vel_rms)), lambda, re_lambda, we_est);
    fclose (fd);
    fflush (fd);
  ...
  double tot_area_1 = interface_area(f);
...
event adapt (i++) {
  ...
  if (single_ph == 1) {
    ...
    adapt_wavelet_leave_interface ({u.x,u.y,u.z}, {f},
                   (double[]){uemax, uemax, uemax, femax}, MAX_LEVEL, MIN_LEVEL, 0);
  }
